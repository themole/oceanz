...
bool Stock::has( Wares type ) const {
...
Stock const Stock::putIn( Stock const & stuff ) {
    Stock not_put_in;
    ...
    for( auto good = stuff._stock.begin(); good != stuff._stock.end(); good++ ) {
        ...
        not_put_in.putIn( this->putIn( good->second, good->first ), good->first );
    }
    return not_put_in;
}
...
Stock const Stock::operator+( Stock const & rhs ) const {
    Stock ret;
    ret.setGlobalMax( globalMax() + rhs.globalMax() );
    for( auto good = _maxs.begin(); good != _maxs.end(); good++ ) {
        ret.setMax( good->second + rhs.max( good->first ), good->first );
    }
    ret.putIn( *this );
    ret.putIn( rhs );
    return ret;
...
operator<<( std::ostream & os, Stock const & s ) {
    for( auto wares = s._stock.begin(); wares != s._stock.end(); wares++ ) {
        switch( wares->first ) {
        case FOOD:
            os << "food:\t\t";
            break;
