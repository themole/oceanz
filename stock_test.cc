...
void printStock( std::string const &,
                 Stock const & );
...
int main() {
    ...
    printStock( "Stock 1", s1 );
    std::cout << " ... not taken out: \t" << not_taken
        << " slaves" << std::endl << std::endl;
    ...
    s2.putIn( 20, SLAVES ); s2.putIn( 10, WOOD );
    s2.putIn( 11, FOOD );
    printStock( "Stock 2", s2 );

    std::cout << " ... putting as much as is in Stock 1 into Stock 2" << std::endl;
    s2.putIn( s1 );
    printStock( "Stock 1", s1 );
    printStock( "Stock 2 = Stock 2 + Stock 1", s2 );

    std::cout << " ... trying to take as much as is in Stock 2 out of Stock 1" << std::endl;
    ...
    std::cout << " ... trying to put as much in Stock 1 as is in Stock 2" << std::endl;
...
void printStock( std::string const & name,
                 Stock const & stock ) {
    if( stock.empty() )
